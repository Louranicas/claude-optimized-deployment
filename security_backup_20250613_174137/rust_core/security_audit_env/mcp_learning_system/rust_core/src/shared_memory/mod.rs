...
impl SharedMemoryRegion {
    ...
    pub async fn write_message(&self, message: &ProtocolMessage) -> Result<()> {
        // Serialize message
        let data = bincode::serialize(message)
            .map_err(|e| CoreError::serialization(format!("Failed to serialize message: {}", e)))?;
        
        // Write to ring buffer
        self.ring_buffer.write(&data)?;
        
        debug!("Wrote message to shared memory: {:?}", message.id);
        Ok(())
    }
    ...
    pub async fn read_message(&self) -> Result<Option<ProtocolMessage>> {
        if let Some(data) = self.ring_buffer.read()? {
            let message: ProtocolMessage = bincode::deserialize(&data)
                .map_err(|e| CoreError::serialization(format!("Failed to deserialize message: {}", e)))?;
            
            debug!("Read message from shared memory: {:?}", message.id);
            Ok(Some(message))
        } else {
            Ok(None)
        }
