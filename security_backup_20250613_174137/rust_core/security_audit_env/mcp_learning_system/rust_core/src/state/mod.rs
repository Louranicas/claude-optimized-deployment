...
pub struct StateManager {
    /// State storage
    state: Arc<DashMap<String, StateEntry, ahash::RandomState>>,
    /// Version counter
...
impl StateManager {
    ...
    pub fn new(max_size: usize) -> Result<Self> {
        ...
        Ok(Self {
            state: Arc::new(DashMap::with_hasher(ahash::RandomState::new())),
    ...
    #[instrument(skip_all, fields(key = %key, value_size = value.len()))]
    pub async fn set(&self, key: String, value: Bytes) -> Result<()> {
        self.set_with_ttl(key, value, self.config.default_ttl).await
    }
    
    /// Set a value with custom TTL
    #[instrument(skip_all, fields(key = %key, value_size = value.len(), ttl = ttl))]
    pub async fn set_with_ttl(&self, key: String, value: Bytes, ttl: u64) -> Result<()> {
