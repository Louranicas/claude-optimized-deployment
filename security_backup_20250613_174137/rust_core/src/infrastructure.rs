    ...
    fn scan_services_adaptive(&self, py: Python, targets: Vec<(String, u16)>) -> PyResult<Vec<bool>> {
        py.allow_threads(|| {
            ...
            runtime.block_on(async {
                self.scan_services_async_impl(targets).await
            })
        })
    }
    
    /// Async scanning implementation with connection pooling and rate limiting
    async fn scan_services_async_impl(&self, targets: Vec<(String, u16)>) -> PyResult<Vec<bool>> {
        let timeout = Duration::from_millis(self.timeout_ms);
        let cache = Arc::clone(&self.results_cache);
