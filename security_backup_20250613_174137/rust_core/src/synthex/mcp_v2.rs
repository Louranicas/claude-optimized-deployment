...
impl McpV2Connection {
    ...
    /// Send a message
    pub async fn send_message(&mut self, msg_type: MessageType, message: &Message) -> Result<(), Box<dyn std::error::Error>> {
        // Serialize message
    ...
    async fn write_header(&mut self, header: &MessageHeader) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn read_header(&mut self) -> Result<MessageHeader, Box<dyn std::error::Error>> {
    ...
    /// Decompress payload
    fn decompress_payload(&self, data: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
pub trait MessageHandler: Send + Sync {
    async fn handle(&self, message: Message) -> Result<Option<Message>, Box<dyn std::error::Error>>;
}
...
impl McpV2Client {
    ...
    /// Connect to server
    pub async fn connect(&self, name: &str, addr: &str) -> Result<(), Box<dyn std::error::Error>> {
