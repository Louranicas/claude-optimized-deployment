...
impl KnowledgeGraph {
    ...
    pub fn add_entity(&self, entity: Entity) -> Result<NodeIndex, Box<dyn std::error::Error>> {
    ...
    pub fn update_from_results(&mut self, results: &SearchResult) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn export_to_json(&self) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
        let graph = self.graph.read();
