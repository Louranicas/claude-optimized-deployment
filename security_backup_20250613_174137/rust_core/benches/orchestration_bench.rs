...
use claude_optimized_deployment_rust::{
    orchestrator::{ServiceMetadata, DeploymentState, HealthStatus, ResourceUsage, NetworkConfig, Protocol},
    services::ServiceRegistry,
    resources::{ResourceManager, ResourceRequest, CpuManager, MemoryManager, StorageManager},
    network::{PortAllocator, ServiceMesh, MeshConfig},
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                let allocation = black_box(
                    manager.allocate_resources(&service_id, request).await.unwrap()
                );
    ...
    c.bench_function("cpu_allocation", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                let service_id = Uuid::new_v4();
                let allocation = black_box(
                    cpu_manager.allocate(&service_id, 2.0).await.unwrap()
                );
    ...
    c.bench_function("memory_allocation", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                let service_id = Uuid::new_v4();
                let allocation = black_box(
                    memory_manager.allocate(&service_id, 1024).await.unwrap()
                );
    ...
    c.bench_function("port_allocation", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                let service_id = Uuid::new_v4();
                let allocation = black_box(
                    allocator.allocate_port(&service_id, Protocol::Http).await.unwrap()
                );
