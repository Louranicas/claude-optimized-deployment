...
use claude_optimized_deployment_rust::{
    ...
    services::{HealthChecker, HealthCheckConfig, HealthCheckType},
    network::Protocol,
    reliability::{CircuitBreaker, CircuitBreakerConfig, RetryPolicyBuilder, BackoffStrategy},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            match engine_clone.deploy_service(
                service_name.clone(),
                "1.0.0".to_string(),
                resources,
            ).await {
    ...
    let result: Result<String, Box<dyn std::error::Error>> = retry_policy.execute(|| async {
        attempt_count += 1;
        if attempt_count < 3 {
