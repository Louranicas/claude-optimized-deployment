...
use claude_optimized_deployment_rust::{
    orchestrator::{OrchestrationEngine, EngineConfig, DeploymentState},
    resources::{ResourceRequest, ResourceManager},
    services::{ServiceRegistry, HealthChecker, HealthCheckConfig, LifecycleManager},
    network::{NetworkAllocator, Protocol},
    reliability::{CircuitBreaker, RetryPolicy, RecoveryManager, RecoveryStrategy},
};
use std::sync::Arc;
...
async fn test_resource_management() {
    ...
    let alloc1 = manager.allocate_resources(&service1, request1).await.unwrap();
    let alloc2 = manager.allocate_resources(&service2, request2).await.unwrap();
...
async fn test_network_allocation() {
    ...
    let config1 = allocator.allocate_port(&service1, Protocol::Http).await.unwrap();
    let config2 = allocator.allocate_port(&service2, Protocol::Grpc).await.unwrap();
...
async fn test_circuit_breaker_integration() {
    use claude_optimized_deployment_rust::reliability::{CircuitBreakerConfig, with_circuit_breaker};
...
async fn test_graceful_shutdown() {
    let engine = OrchestrationEngine::new(EngineConfig::default()).await.unwrap();
