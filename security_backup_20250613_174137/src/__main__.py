...
from src.core.secrets_manager import get_secrets_manager, SecretNotFoundError
...
def main():
    ...
    try:
        ...
        try:
            # Try to get a test secret to verify Vault is accessible
            secrets_manager.get_secret("test/health")
            logger.info("Successfully connected to Vault")
        except SecretNotFoundError:
            logger.info("Vault is accessible (test secret not found)")
        except Exception as e:
            logger.warning(f"Vault not accessible, using environment variables: {e}")
