#!/usr/bin/env python3
"""
AGENT 6 - DATA FLOW AND INTEGRATION VALIDATION (FIXED VERSION)
===============================================================
...
try:
    from test_agent6_additional_validators import AdditionalValidationHelpers, NetworkConnectivityValidator
    ADDITIONAL_VALIDATORS_AVAILABLE = True
except ImportError:
    ADDITIONAL_VALIDATORS_AVAILABLE = False
    print("Warning: Additional validators not available, using simplified tests")
...
try:
    ...
    print("Warning: httpx not available, skipping HTTP tests")
...
try:
    ...
    print("Warning: pyyaml not available, skipping YAML config tests")
...
class Agent6DataFlowIntegrationValidator:
    ...
    def __init__(self):
        ...
        if ADDITIONAL_VALIDATORS_AVAILABLE:
            self.additional_validators = AdditionalValidationHelpers()
            self.network_validator = NetworkConnectivityValidator()
        else:
            self.additional_validators = None
    ...
    async def validate_data_flows(self) -> List[DataFlowValidationResult]:
        ...
        flows.extend(await self._validate_database_flows())
        
        # 4. Validate Cache data flows
        flows.extend(await self._validate_cache_flows())
        
        # 5. Validate File system data flows
        flows.extend(await self._validate_filesystem_flows())
        
        # 6. Validate Inter-service communication flows
        flows.extend(await self._validate_interservice_flows())
            ...
            if config_path.exists():
                if YAML_AVAILABLE:
                    ...
                    return DataFlowValidationResult(
                        ...
                        latency_ms=(time.time() - start_time) * 1000,
                        data_size_bytes=file_size,
                        warnings=["YAML parser not available, only checked file existence"],
    ...
    async def _validate_api_gateway_flows(self) -> List[DataFlowValidationResult]:
        ...
        if not self.http_client:
            flows.append(DataFlowValidationResult(
                ...
                latency_ms=0,
                data_size_bytes=0,
                errors=["HTTP client not available"]
            ))
            return flows
    ...
    async def _validate_database_flows(self) -> List[DataFlowValidationResult]:
    ...
    async def _validate_cache_flows(self) -> List[DataFlowValidationResult]:
    ...
    async def _validate_filesystem_flows(self) -> List[DataFlowValidationResult]:
    ...
    async def _validate_interservice_flows(self) -> List[DataFlowValidationResult]:
    ...
    async def validate_api_endpoints(self) -> List[APIValidationResult]:
        ...
        # 3. Additional endpoints if validators available
        if self.additional_validators:
            try:
                endpoints.extend(await self.additional_validators._test_mcp_server_endpoints())
                endpoints.extend(await self.additional_validators._test_auth_endpoints())
                endpoints.extend(await self.additional_validators._test_data_processing_endpoints())
            except Exception as e:
                endpoints.append(APIValidationResult(
                    endpoint="additional_validators",
                    method="GET",
                    status_code=0,
                    response_time_ms=0,
                    payload_size_bytes=0,
                    response_size_bytes=0,
                    validation_status="FAILED",
                    errors=[f"Additional validator error: {str(e)}"]
                ))
    ...
    async def validate_database_integrations(self) -> List[DatabaseValidationResult]:
        ...
        # 2. Test additional databases if validators available
        if self.additional_validators:
            try:
                databases.extend(await self.additional_validators._test_postgresql_integration())
                databases.extend(await self.additional_validators._test_redis_integration())
                databases.extend(await self.additional_validators._test_database_transactions())
            except Exception as e:
                databases.append(DatabaseValidationResult(
                    database_type="additional_validation",
                    connection_string="",
                    operation_type="ADDITIONAL_TESTS",
                    query="",
                    execution_time_ms=0,
                    rows_affected=0,
                    validation_status="FAILED",
                    errors=[f"Additional database tests failed: {str(e)}"]
                ))
    ...
    async def validate_external_services(self) -> List[ExternalServiceValidationResult]:
        ...
        # 3. Test additional services if validators available
        if self.additional_validators:
            try:
                services.extend(await self.additional_validators._test_aws_integration())
                services.extend(await self.additional_validators._test_monitoring_services())
            except Exception as e:
                services.append(ExternalServiceValidationResult(
                    service_name="additional_services",
                    service_type="validation",
                    endpoint="",
                    authentication_method="none",
                    response_time_ms=0,
                    validation_status="FAILED",
                    errors=[f"Additional service tests failed: {str(e)}"]
                ))
    ...
    async def validate_event_systems(self) -> List[Dict[str, Any]]:
        ...
        events.append({
            ...
            "latency_ms": 0,
            "errors": [],
            "metadata": {"note": "Basic event system validation initialized"}
        })
        
        # Additional event tests if validators available
        if self.additional_validators:
            try:
                events.extend(await self.additional_validators._test_redis_pubsub())
                events.extend(await self.additional_validators._test_celery_messaging())
                events.extend(await self.additional_validators._test_webhook_systems())
            except Exception as e:
                events.append({
                    "system_type": "additional_event_tests",
                    "test_type": "error",
                    "status": "FAILED",
                    "latency_ms": 0,
                    "errors": [f"Additional event tests failed: {str(e)}"],
                    "metadata": {"error_type": type(e).__name__}
                })
        ...
        try:
            ...
            report = {
                ...
                "system_capabilities": {
                    ...
                    "additional_validators_available": ADDITIONAL_VALIDATORS_AVAILABLE
                },
                "recommendations": [
