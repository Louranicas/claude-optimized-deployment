...
# Get the correct ELB service account for the region
...
resource "aws_lb_target_group" "metrics" {
  name     = "${var.cluster_name}-metrics-tg"
  port     = 9090
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
...
resource "aws_lb_listener_rule" "metrics" {
  ...
  # Restrict metrics access to internal IPs only
  condition {
    source_ip {
      values = [var.vpc_cidr]
    }
...
output "target_group_metrics_arn" {
  description = "Metrics target group ARN"
  value       = aws_lb_target_group.metrics.arn
}
