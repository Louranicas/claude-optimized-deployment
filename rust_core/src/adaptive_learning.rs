    ...
    fn batch_process_patterns(&self, py: Python<'_>,
        ...
        let results: Vec<f64> = (0..num_patterns)
            ...
            .map(|i| {
                ...
                self.process_single_pattern_optimized(pattern.to_slice()?)
            })
            .collect::<Vec<f64>>();
...
impl RustLearningCore {
        ...
        knowledge_list.iter()
            .filter_map(|item| {
                if let Ok(dict) = item.downcast::<PyDict>() {
                    ...
                    let knowledge_type = dict.get_item("type")
                        .ok()
                        .flatten()
                        .and_then(|v| v.extract::<String>().ok())?;
                    let data = dict.get_item("data")
                        ...
                        .unwrap_or_default();
                    let relevance_score = dict.get_item("relevance")
                        .ok()
                        .flatten()
    ...
    fn extract_performance_history(&self, history_list: &PyList) -> PyResult<Vec<PerformancePoint>> {
        Ok(history_list.iter()
            .filter_map(|item| {
                if let Ok(dict) = item.downcast::<PyDict>() {
                    let accuracy = dict.get_item("accuracy")
                        .ok()
                        .flatten()
                        .and_then(|v| v.extract::<f64>().ok())?;
                    let loss = dict.get_item("loss")
                        .ok()
                        .flatten()
                        .and_then(|v| v.extract::<f64>().ok())
                        .unwrap_or(1.0);
    ...
    fn convert_patterns_to_python(&self, py: Python<'_>, patterns: &[ProcessedPattern]) -> PyResult<PyObject> {
        let py_list = PyList::new(py, patterns.iter().map(|pattern| {
            ...
            dict.set_item("pattern_id", &pattern.pattern_id).expect("Unexpected None/Error");
            dict.set_item("confidence", pattern.confidence).expect("Unexpected None/Error");
            dict.set_item("enhancement_score", pattern.enhancement_score)?;
            dict
        }));
...
#[pymodule]
pub fn adaptive_learning(_py: Python, m: &PyModule) -> PyResult<()> {
