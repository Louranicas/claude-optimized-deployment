...
use super::*;
use crate::synthex::parallel_executor::RawSearchResults;
...
impl ResultAggregator {
    ...
    pub async fn aggregate(&self, raw_results: Vec<RawSearchResults>) -> Result<SearchResult> {
        let start_time = std::time::Instant::now();
        
        // Flatten all results
        let mut all_results: Vec<(agents::RawResult, String)> = raw_results
            .par_iter()
            .flat_map(|raw| {
                raw.results
                    .iter()
...
impl Deduplicator {
    ...
    fn deduplicate(&self, results: &mut [(agents::RawResult, String)]) -> Vec<(agents::RawResult, String)> {
...
impl ResultRanker {
    ...
    fn rank(&self, mut results: Vec<(agents::RawResult, String)>) -> Vec<(agents::RawResult, String)> {
    ...
    fn calculate_composite_score(&self, result: &agents::RawResult, source: &str, current_time: u64) -> f64 {
...
impl SemanticClusterer {
    ...
    fn cluster(&self, results: Vec<(agents::RawResult, String)>) -> Result<Vec<ResultCluster>> {
        if results.is_empty() {
            return Ok(vec![]);
        }
    ...
    fn generate_cluster_label(&self, cluster: &[(agents::RawResult, String)]) -> String {
...
struct ResultCluster {
    label: String,
    results: Vec<(agents::RawResult, String)>,
    avg_score: f64,
}
