...
// Helper macro for hashmaps - must be defined before use
macro_rules! hashmap {
    ($($key:expr => $value:expr),*) => {
        {
...
impl WebSearchAgent {
    ...
    async fn search_brave(&self, query: &str) -> Result<Vec<RawResult>> {
    ...
    async fn search_searxng(&self, query: &str) -> Result<Vec<RawResult>> {
    ...
    async fn scrape_url(&self, url: &str) -> Result<RawResult> {
