...
impl ApiSearchAgent {
    ...
    async fn execute_with_retry(
        ...
        loop {
            ...
            match request.try_clone().expect("Unexpected None/Error").send().await {
    ...
    fn extract_path<'a>(&self, json: &'a Value, path: &str) -> Result<&'a Value> {
...
impl SearchAgent for ApiSearchAgent {
    ...
    async fn search(
        ...
        all_results.sort_by(|a, b| b.score.partial_cmp(&a.score).expect("Unexpected None/Error"));
    ...
    async fn health_check(&self) -> Result<HealthStatus> {
        ...
        let (_, endpoint) = endpoints.iter().next().expect("Unexpected None/Error");
        let start = std::time::Instant::now();
