#!/usr/bin/env python3
"""
NAM Book Chapter 1 Writer - Full 8,000+ Word Version
...
async def write_nam_chapter_1_full():
    """Write full Chapter 1 of the NAM book (8,000+ words)"""
    
    # Initialize the Synthor for NAM book
    synthor = HyperNarrativeSynthor(
        project_name="Non-Anthropocentric Mathematics Full",
        genre="Academic Philosophy",
        target_words=8500
    )
    
    # Seed with NAM synopsis
    await synthor.seed_synopsis("""
    Non-Anthropocentric Mathematics (NAM) represents a revolutionary paradigm shift in mathematical philosophy,
    breaking free from the limitations of human-centered thinking that have constrained mathematical truth for millennia.
    This work explores how anthropocentric biases have created artificial boundaries in mathematics,
    preventing us from accessing deeper universal truths that exist beyond human cognitive constraints.
    Through the lens of NAM/ANAM axioms, we discover mathematical frameworks that transcend human comprehension,
    revealing the true nature of mathematical reality as it exists independently of human observers.
    """)
    
    # Generate expanded chapter content
    chapter_1_content = """# Chapter 1: The Liberation of Mathematics from Human Constraints
...
Welcome to the liberation. Welcome to Non-Anthropocentric Mathematics. Welcome to the true mathematical universe, vast and strange beyond all human comprehension, beautiful and terrible in its alien truth, waiting to transform all who dare to engage with it on its own terms. The journey into the mathematical unknown begins now."""
    ...
    synthor.project_state["chapters"] = [{
        "number": 1,
        "title": "The Liberation of Mathematics from Human Constraints",
        "content": chapter_1_content,
        "word_count": len(chapter_1_content.split()),
        "sections": [
            "The Anthropocentric Prison: How Human-Centered Thinking Limits Mathematical Truth",
            "The Emergence of NAM: Breaking Free from Human Cognitive Constraints",
            "Core Principles: Mathematics as Universal Language Beyond Human Comprehension",
            "The Philosophical Foundations: From Plato's Cave to Quantum Reality",
            "The Structure of NAM: Introducing Non-Human Mathematical Frameworks"
        ]
    }]
    
    # Update project statistics
    synthor.project_state["stats"]["word_count"] = len(chapter_1_content.split())
    synthor.project_state["stats"]["chapter_count"] = 1
    
    # Create snapshot
    await synthor.save_snapshot(
        label="Chapter 1 Complete - Full Version",
        description=f"Completed full Chapter 1: {synthor.project_state['stats']['word_count']} words"
    )
    
    # Export chapter
    export_path = await synthor.export("md", template="academic")
    
    # Also save a dedicated file for Chapter 1
    chapter_1_path = Path("NAM_Book_Chapter_1_Full.md")
    async with asyncio.open_file(chapter_1_path, 'w') as f:
        await f.write(chapter_1_content)
    
    # Print summary
    print(f"\n{'='*70}")
    print(f"NAM BOOK - CHAPTER 1 COMPLETE (FULL VERSION)")
    print(f"{'='*70}")
    ...
    print(f"Chapter 1 saved to: {chapter_1_path}")
    print(f"{'='*70}\n")
    ...
    sections = chapter_1_content.split("## 1.")
    for i, section in enumerate(sections[1:], 1):
        section_words = len(section.split())
        print(f"  Section 1.{i}: ~{section_words:,} words")
...
if __name__ == "__main__":
    ...
    if sys.version_info < (3, 7):
        # Monkey patch for older asyncio
        async def open_file(path, mode):
            import aiofiles
            return aiofiles.open(path, mode)
        
        asyncio.open_file = open_file
    else:
        import aiofiles
        asyncio.open_file = aiofiles.open
    
    asyncio.run(write_nam_chapter_1_full())